...
def cronjobDetectGas():
    gas = fetchEtherscanGas()
    lowPrice = gas["lowPrice"]
    avgPrice = gas["avgPrice"]
    highPrice = gas["highPrice"]
    
    log = '%s low: %s avg: %s high: %s  ' %(datetime.now().strftime('%Y-%m-%d %H:%M:%S'), lowPrice, avgPrice, highPrice)
    
    if float(highPrice) <= GAS_LIMIT:
        nowTime = time.time()
        with open('/mnt/c/Users/henry/bot/time.txt', 'r') as rf:
            pastTime = float(rf.read() or 0);
            if nowTime - float(pastTime) >= PASS_TIME * 60:
                print('send')
                sendText(f'Low: {lowPrice}\nAverage: {avgPrice}\nHigh: {highPrice}')
                log += 'send'
                with open('/mnt/c/Users/henry/bot/time.txt', 'w') as wf:
                    wf.write(str(nowTime))
    log += '\n'
    with open('/mnt/c/Users/henry/bot/gwei_log.txt', 'a') as af:
        af.write(log)
